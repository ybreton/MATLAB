...
if ~exist('fd','var'); fd = findfiles('*._t'); end;
fc=[fc; fd];
...
for iFC = 1:length(fc)
	...
	if ~recalcYN % if we should try to calculate the CQ file	
		...
		if isempty(fc_TT); % if we still do not have a tetrode file, we cant' create the CQ file
			...
			f_cluster(find(f_cluster > nS)) = NaN;
			...
			if badTT % if we got the wrong data loaded
				...
				% Find .fd file  -- ADR 2012-12-12
				[ttfd, ttfn, ttxt] = fileparts(fc_TT{1});
				fdFile = fullfile(ttfd, 'FD', [ttfn '.fd']);
				if exist(fdFile, 'file');
					fprintf('Using fd file: %s\n', fdFile);
					load(fdFile, 'ChannelValidity', '-mat');
					fprintf('ChannelValidity'); fprintf('%d ', ChannelValidity); fprintf('\n');
					nCh = sum(ChannelValidity);
				else
					fprintf('fdFile not found. Assuming all channels valid.');
					ChannelValidity = true(nCh,1);					
				end
				...
				if ~strcmpi(Name.Location,fd_Curr) | ~strcmpi(Name.Tetrode,TT_Curr)
					if isempty(WVD)
						...
						for iB = 1:nB
							...
							f_get = f_get(find(f_get <= nS));
							
							[t,WVD] = MClust_LoadNeuralData(fc_TT{1},f_get,2);
							
							WVD = WVD(:, logical(ChannelValidity), :);  % ADR 2012-12-12 
