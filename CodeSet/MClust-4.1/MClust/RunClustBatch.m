...
% featuresToUse = {'feature_Energy', 'feature_EnergyD1' 'feature_Peak'};
% SubSetAt = 1e6;
...
if isempty(fcTT)
    ...
    include = true(length(fcTT),1);
    for iTT=1:length(fcTT)
        WV = LoadTT_NeuralynxNT(fcTT{iTT});
        nSpikes = length(WV);
        include(iTT) = nSpikes>1;
    end
    fcTT = fcTT(include);
end
