function [ PhaseW ] = DD_getPhaseSW(sd,varargin )
% 2012-02-20 AndyP
...
%------------------
PhaseW = nan(nL,1);
if isempty(TA); [TA,UpDn,~] = DD_getLapType(sd,'type',1,'TorA','T','nW',nW,'nL',nL);
end
	for iL=nW:nW:nL % step through each window
		W=TA(iL-nW+1,1):TA(iL,1); %get window
		if nansum(W)/length(W)>thr & UpDn(iL)~=2; %#ok<AND2> exclude laps where the adjusting delay is floored and is not changing
			PhaseW(iL-nW+1:iL,1)=2; % classify titration phase
		else
			PhaseW(iL-nW+1:iL,1)=3; % classify exploitation phase
		end
	end
	T1 = min(find(PhaseW==2,1,'first'), maxI);	PhaseW(1:T1-1)=1; % classify investigation phase
end
