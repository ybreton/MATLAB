function RRPromote_auto(varargin)
...
for d = 1 : length(fd)
    ...
    TetrodeExists = ~isempty(FindFiles('*.ntt','CheckSubdirs',false));
    if TetrodeExists
        Protocol = 'HyperDrive';
    else
        Protocol = 'Behavior';
...
%             Toff = max(x.range);
        ...
        clf
        [x,y]=LoadVT_lumrg(nvt{1});
        plot(x.range,atan2(y.data-nanmean(y.data),x.data-nanmean(x.data)))
        hold on
        plot([Ton Ton],[-pi pi],'g-','linewidth',2)
        plot([Toff Toff],[-pi pi],'r-','linewidth',2)
