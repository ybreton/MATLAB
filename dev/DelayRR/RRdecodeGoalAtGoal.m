function Decoding = RRdecodeGoalAtGoal(sd,S,varargin)
...
for iTrial=1:length(sd.ZoneIn);
    ...
    % When he entered
    In = sd.ExitZoneTime(iTrial);
    % When he left
    if iTrial>length(sd.NextZoneTime)
        Out = sd.ExpKeys.TimeOffTrack;
    else
        Out = sd.NextZoneTime(iTrial);
    end
