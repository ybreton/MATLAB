...
PostFeed = [];
nPellets = [2 2 2 2];       % the number of pellets delivered at each zone
...
%----------------
if isempty(Behavior);
		Behavior = input('Enter Behavioral Task [STABLE RR]:            ', 's');
		if isempty(Behavior); Behavior = 'STABLE RR'; end
end
if isempty(Condition)
    Condition = input('Experimental condition [NONE]:       ','s');
    if isempty(Condition); Condition = ''; end
end
if ~isempty(Condition) && isempty(Dose)
    Dose = input('Condition Dose [0]:       ');
...
if strcmp(Protocol,'Hyperdrive');
    ...
    for c = 1 : length(space)-1
        CSC{c} = CSCReference(space(c):space(c+1));
    end
...
fprintf(fid,'ExpKeys.Dose = %.2f;\n',Dose);
fprintf(fid,'ExpKeys.Protocol = ''%s'';\n',Protocol);
% fprintf(fid,'ExpKeys.PelletRatio = %d;\n',PelletRatio);
% for z = 1 : length(nPellets)
%     fprintf(fid,'ExpKeys.PelletRatio(%d) = %d;\n',z,nPellets(z));
% end
for z = 1 : size(FeederDelayList,1)
    fprintf(fid,'ExpKeys.FeederDelayList = [ ');
    for delay = 1 : length(FeederDelayList)
        fprintf(fid,'%.1f ',FeederDelayList(delay));
    end
    fprintf(fid,'];\n');
end
for z = 1 : size(FeederProbList,1)
    fprintf(fid,'ExpKeys.FeederProbList = [');
    for prob = 1 : size(FeederProbList,2)
        fprintf(fid,'%.2f ',FeederProbList(prob));
    end
    fprintf(fid,'];\n');
end
...
fprintf(fid,'ExpKeys.TimeOnTrack = %d;\n',min(TimeOnTrack(~isnan(TimeOnTrack))));
fprintf(fid,'ExpKeys.TimeOffTrack = %d;\n',max(TimeOffTrack(~isnan(TimeOffTrack))));
