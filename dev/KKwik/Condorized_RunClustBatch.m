function Condorized_RunClustBatch(batchfile,varargin)
%
...
% MvdM 2007 added submit to condor option (use 'SubmitToCondor',1 as arguments)
%
...
if strcmp(Do_AutoClust,'yes')
   ...
   if strcmpi('KlustaKwik',gPar.ClusterAlgorithm)
       KlustaKwikPath = which('KlustaKwik-1_5.exe');
       ...
       elseif(strcmpi(gPar.ClusterAlgorithm,'KlustaKwik')) 
           ...
           parameter_string = ['-MinClusters ' num2str(fPar{i}.KKwikMinClusters) ...
			   ...
			   if SubmitToCondor
				   ...
				   %disp(KlustaKwikPath)
				   %disp(file_no)
				   %disp(parameter_string)
				   
				   % find what the .fet.N filename is without path
				   % (this will be input file transfer argument): this
				   % should be IN without path plus .fet. plus file_no
				   
				   % get filename without path
				   filesepind = regexpi(IN,'\');
				   barefile = IN(filesepind(end)+1:end);
				   barefile_fet = [barefile '.fet.' num2str(file_no)];
				   file_fet = [IN '.fet.' num2str(file_no)];
				   
				   job_fname = [barefile '.cdr'];
				   
				   cd FD % assumes FD? check if this is bad.
				   
				   if ~exist(job_fname,'file')
					   fid = fopen(job_fname,'w');
					   
					   % now write
					   fprintf(fid,'executable = %s\n',KlustaKwikPath);
					   fprintf(fid,'universe = vanilla\n');
					   fprintf(fid,'arguments = %s %s %s\n',barefile,num2str(file_no),parameter_string);
					   fprintf(fid,'output = %s.out\n',barefile);
					   fprintf(fid,'error = %s.error\n',barefile);
					   fprintf(fid,'log = %s.log\n',barefile);
					   fprintf(fid,'Requirements = (OpSys == "WINNT51" || OpSys =="WINNT61")\n');
					   ...
					   fclose(fid);
					   COMMAND = ['!condor_submit ' job_fname];
					   COMD_output = evalc(COMMAND)
				   else
					   disp(sprintf('File %s exists, skipping...',job_fname));
           ...
           if ~SubmitToCondor % problem: if using condor, .fet files hang around.
