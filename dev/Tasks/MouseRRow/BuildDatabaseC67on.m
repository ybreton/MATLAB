function BuildDatabaseC67on(RawData,setmouse,setday,setpreweight,setpostweight,feederlocation,leaveborder,a,d)
...
taskEvents.EnterTimeStampLog (RawData{animal,day}(:,11)==0 & RawData{animal,day}(:,16)==0 & RawData{animal,day}(:,21)==0 & RawData{animal,day}(:,26)==0) = NaN;
...
taskEvents.SkipTimeStampLog (RawData{animal,day}(:,10)==0 & RawData{animal,day}(:,15)==0 & RawData{animal,day}(:,20)==0 & RawData{animal,day}(:,25)==0) = NaN;
...
taskEvents.QuitTimeStampLog (RawData{animal,day}(:,12)==0 & RawData{animal,day}(:,17)==0 & RawData{animal,day}(:,22)==0 & RawData{animal,day}(:,27)==0) = NaN;
...
taskEvents.EarnTimeStampLog (RawData{animal,day}(:,13)==0 & RawData{animal,day}(:,18)==0 & RawData{animal,day}(:,23)==0 & RawData{animal,day}(:,28)==0) = NaN;
...
while isempty(find(buffer>0))==0 || isempty(find(taskEvents.OfferTimeStamp>taskEvents.EnterTimeStamp))==0 || isempty(find(taskEvents.OfferTimeStamp>taskEvents.EarnTimeStamp))==0
    ...
    %hanging offer at end (animal is in DZ)
    ...
    %hanging countdown at end (animal just entered WZ)
    ...
    if ~isnan(taskEvents.OfferTimeStamp(end)) && isnan(taskEvents.SkipTimeStamp(end)) && ~isnan(taskEvents.EnterTimeStamp(end)) && isnan(taskEvents.QuitTimeStamp(end)) && isnan(taskEvents.EarnTimeStamp(end))
     ...
     %hanging free enter and earn at end
    if isnan(taskEvents.OfferTimeStamp(end)) && isnan(taskEvents.SkipTimeStamp(end)) && ~isnan(taskEvents.EnterTimeStamp(end)) && isnan(taskEvents.QuitTimeStamp(end)) && ~isnan(taskEvents.EarnTimeStamp(end))
        taskEvents.OfferTimeStamp=taskEvents.OfferTimeStamp(1:end-1);
        taskEvents.Flavor=taskEvents.Flavor(1:end-1);
        taskEvents.Offer=taskEvents.Offer(1:end-1);
        ...
        taskEvents.WorkTime=taskEvents.EarnTimeStamp -taskEvents.OfferTimeStamp;
         ...
         %hanging free enter and quit at end
    if isnan(taskEvents.OfferTimeStamp(end)) && isnan(taskEvents.SkipTimeStamp(end)) && ~isnan(taskEvents.EnterTimeStamp(end)) && ~isnan(taskEvents.QuitTimeStamp(end)) && isnan(taskEvents.EarnTimeStamp(end))
        taskEvents.OfferTimeStamp=taskEvents.OfferTimeStamp(1:end-1);
        taskEvents.Flavor=taskEvents.Flavor(1:end-1);
        taskEvents.Offer=taskEvents.Offer(1:end-1);
...
%calculate distance and leave properties
...
taskEvents.DistanceFromEarnSpotToLeave=NaN(length(taskEvents.EarnTimeStamp),1);
taskEvents.LeaveTimeStamp=NaN(length(taskEvents.EarnTimeStamp),1);
taskEvents.DistanceFromFeederToEarnSpot=NaN(length(taskEvents.EarnTimeStamp),1);

for E=1:length(taskEvents.EarnTimeStamp)

    if isnan(taskEvents.EarnTimeStamp(E))
        taskEvents.DistanceFromEarnSpotToLeave(E,1)=NaN;
        taskEvents.LeaveTimeStamp(E,1)=NaN;
        taskEvents.DistanceFromFeederToEarnSpot(E,1)=NaN;

        continue
        
    else
        earnposx=x.D(x.T==taskEvents.EarnTimeStamp(E));
        earnposx=earnposx(1);
        
        earnposy=y.D(y.T==taskEvents.EarnTimeStamp(E));
        earnposy=earnposy(1);
        
        
        taskEvents.DistanceFromFeederToEarnSpot(E,1)=pdist([earnposx,earnposy;feederlocation.(keys.Flavors{(taskEvents.Flavor(E)) })]);
        taskEvents.DistanceFromEarnSpotToLeave(E,1)=abs( det([([earnposx,earnposy])-leaveborder.R.(keys.Flavors{(taskEvents.Flavor(E)) });leaveborder.L.(keys.Flavors{(taskEvents.Flavor(E)) })-leaveborder.R.(keys.Flavors{(taskEvents.Flavor(E)) })]) )/norm(leaveborder.L.(keys.Flavors{(taskEvents.Flavor(E)) })-leaveborder.R.(keys.Flavors{(taskEvents.Flavor(E)) }));
        
        scanstart=find(x.T==taskEvents.EarnTimeStamp(E));
        scanstart=scanstart(1);
        scanend=scanstart+(length(x.T)-1);
        tempdistancelist=NaN(length(x.T),1);
    
        for trypos=scanstart:scanend
            if trypos > length(x.D)
                continue
            else
                ...
                tempdistancelist(trypos+1-scanstart,1)=( det([([tryposx,tryposy])-leaveborder.R.(keys.Flavors{(taskEvents.Flavor(E)) });leaveborder.L.(keys.Flavors{(taskEvents.Flavor(E)) })-leaveborder.R.(keys.Flavors{(taskEvents.Flavor(E)) })]) )/norm(leaveborder.L.(keys.Flavors{(taskEvents.Flavor(E)) })-leaveborder.R.(keys.Flavors{(taskEvents.Flavor(E)) }));
            end
        end
        
        leavescanpos=find(tempdistancelist>0);
        
        if isempty(leavescanpos) && (length(tempdistancelist(~isnan(tempdistancelist)))~=length(tempdistancelist))
            taskEvents.LeaveTimeStamp(E,1)=x.T(end);
        else
            leavescanpos=leavescanpos(1);
            taskEvents.LeaveTimeStamp(E,1)=x.T(scanstart+(leavescanpos-1));
...
taskEvents.LeaveTime=taskEvents.LeaveTimeStamp-taskEvents.EarnTimeStamp;
