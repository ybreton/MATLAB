function RatTrackRunFrames3(mmStruct,frameRange)
...
nIter = 0;
for f = frameRange
    mmStructRat = mmStruct;
    frame = read(mmStruct.vidObj,f);
    mmStructRat.frame.cdata = double(frame);
    %mmStructRat = addVidFrame3(mmStruct,f);
    %ProcessRatFrame3(mmStructRat.frames, mmStructRat.width, mmStructRat.height, mmStructRat.frameList, mmStructRat.videoTimes)
    ProcessRatFrame3(mmStructRat.frame, mmStructRat.width, mmStructRat.height, f, f*1./mmStructRat.rate)
    RatTrackData.lastFrame = double(frame);
    nIter = nIter+1;
    if nIter==1
        t1 = clock;
        elapsed=etime(t1,t0);
        remaining = elapsed*(length(frameRange)-nIter);
        fprintf('\nRat tracking %s',mmStruct.filename);
        fprintf('\n\tEach dot is %d frames (%.1fs)',onePercent, onePercent/30)
        fprintf('\n')
...
%         fprintf(' %s\n',str)
    ...
    if mod(nIter,onePercent)==0
        ...
        if RatTrackParms.showProg
            hold on
            plot(f/30/60,10,'rx','markersize',10)
            hold off
        end
    end
...
%         fprintf('Endtime Approx %s\n',str)
        if RatTrackParms.showProg
            hold on
            plot([f/30/60 f/30/60],[0 10],'r-','linewidth',2)
            hold off
        end
